...
const state = {
  ...
  voteCount: 0,
  voteTarget: 0,
  judgePass: "", // mot de passe défini au départ
};
...
function requireJudge(actionLabel){
  ...
  if (input === state.judgePass) return true;
  alert("Accès refusé. Seul le Juge peut faire ça.");
  return false;
}
...
function lockVotes(){
  ...
  $("#voteHint").innerHTML =
    "Votes : <strong>COMPLETS</strong> — Le Tribunal attend le délibéré du Juge.";
}
...
function unlockVotes(){
  ...
  updateVoteUI();
  $("#voteHint").innerHTML =
    `Votes : <span id="voteCount">${state.voteCount}</span> / <span id="voteTarget">${state.voteTarget}</span>
     — Clique “Valider le vote”. Quand le quota est atteint, le vote se bloque.`;
  updateVoteUI();
}
...
function init(){
  $("#btnStart").addEventListener("click", () => {
    ...
    const pass = ($("#judgePass").value || "").trim();
    if (!pass){
